...
static int tracefs_permission(struct inode *inode, int mask)
{
	set_tracefs_inode_owner(inode);
	return generic_permission(inode, mask);
}

static int tracefs_getattr(const struct path *path, struct kstat *stat,
...
static int tracefs_setattr(struct dentry *dentry,
			   struct iattr *attr)
{
