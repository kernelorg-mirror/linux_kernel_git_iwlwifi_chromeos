...
#ifndef _MMDVFS_V3_MEMORY_H_
#define _MMDVFS_V3_MEMORY_H_
...
#else
static inline void *mmdvfs_get_mmup_base(void){ return NULL; }
static inline void *mmdvfs_get_vcp_base(void){ return NULL; }
...
#define MEM_BASE \
	(mmdvfs_get_mmup_enable() ? mmdvfs_get_mmup_base() : mmdvfs_get_vcp_base())
#define MEM_VCP_BASE		mmdvfs_get_vcp_base()
...
#define MEM_GENPD_ENABLE_USR(x)	(MEM_BASE + 0x18 + 0x4 * (x))
#define MEM_AGING_CNT_USR(x)	(MEM_BASE + 0x20 + 0x4 * (x))
#define MEM_FRESH_CNT_USR(x)	(MEM_BASE + 0x28 + 0x4 * (x))
#define MEM_FORCE_OPP_PWR(x)	(MEM_BASE + 0x30 + 0x4 * (x))
#define MEM_VOTE_OPP_PWR(x)	(MEM_BASE + 0x40 + 0x4 * (x))
#define MEM_VOTE_OPP_USR(x)	(MEM_BASE + 0x50 + 0x4 * (x))
...
#define MEM_VMM_OPP_VOLT(x)	(MEM_BASE + 0xA0 + 0x4 * (x))
#define MEM_VDISP_AVS_STEP(x)	(MEM_BASE + 0xC0 + 0x4 * (x))
...
#define MEM_USR_OPP_SEC(x, vcp) \
	(((vcp) && MEM_VCP_BASE ? MEM_VCP_BASE : MEM_BASE) + 0xF0 + 0x4 * (x))
#define MEM_USR_OPP_USEC(x, vcp) \
	(((vcp) && MEM_VCP_BASE ? MEM_VCP_BASE : MEM_BASE) + 0x170 + 0x4 * (x))
#define MEM_USR_OPP(x, vcp) \
	(((vcp) && MEM_VCP_BASE ? MEM_VCP_BASE : MEM_BASE) + 0x1F0 + 0x4 * (x))
#define MEM_USR_FREQ(x, vcp) \
	(((vcp) && MEM_VCP_BASE ? MEM_VCP_BASE : MEM_BASE) + 0x270 + 0x4 * (x))

#define MEM_MUX_OPP_SEC(x)	(MEM_BASE + 0x2F0 + 0x4 * (x))
#define MEM_MUX_OPP_USEC(x)	(MEM_BASE + 0x330 + 0x4 * (x))
#define MEM_MUX_OPP(x)		(MEM_BASE + 0x370 + 0x4 * (x))
#define MEM_MUX_MIN(x)		(MEM_BASE + 0x3B0 + 0x4 * (x))
#define MEM_FORCE_CLK(x)	(MEM_BASE + 0x3F0 + 0x4 * (x))

#define MEM_PWR_OPP_SEC(x)	(MEM_BASE + 0x430 + 0x4 * (x))
#define MEM_PWR_OPP_USEC(x)	(MEM_BASE + 0x440 + 0x4 * (x))
#define MEM_PWR_OPP(x)		(MEM_BASE + 0x450 + 0x4 * (x))
#define MEM_PWR_CUR_GEAR(x)	(MEM_BASE + 0x460 + 0x4 * (x))
#define MEM_FORCE_VOL(x)	(MEM_BASE + 0x470 + 0x4 * (x))

#define MEM_VMM_BUCK_ON		(MEM_BASE + 0x480)

#define MEM_PWR_TOTAL_TIME(x, y) \
	(MEM_BASE + 0x484 + 0x40 * (x) + 0x8 * (y))
...
#define MEM_CEIL_LEVEL(x)		(MEM_BASE + 0x598 + 0x4 * (x))
/* next start: 0x5A4 */

#define MEM_REC_CNT_MAX		16

/* POWER_NUM(3), CAM_ALONE,x: POWER_NUM, y: CNT */
#define MEM_REC_PWR_ALN_OBJ		3
#define MEM_REC_PWR_ALN_CNT(x)		(MEM_BASE + 0x7DC + 0xC4 * (x))
#define MEM_REC_PWR_ALN_SEC(x, y) \
	(MEM_BASE + 0x7E0 + 0xC4 * (x) + MEM_REC_PWR_ALN_OBJ * 0x4 * (y))
#define MEM_REC_PWR_ALN_NSEC(x, y) \
	(MEM_BASE + 0x7E4 + 0xC4 * (x) + MEM_REC_PWR_ALN_OBJ * 0x4 * (y))
#define MEM_REC_PWR_ALN_OPP(x, y) \
	(MEM_BASE + 0x7E8 + 0xC4 * (x) + MEM_REC_PWR_ALN_OBJ * 0x4 * (y))
...
#define SRAM_REC_CNT_USR(x)	(SRAM_BASE + 4 * (0 + (x)))
#define SRAM_USR_SEC(x, y) \
	(SRAM_BASE + 4 * (2 + SRAM_OBJ_CNT * ((x) + (y) * SRAM_USR_NUM) + 0))
#define SRAM_USR_USEC(x, y) \
	(SRAM_BASE + 4 * (2 + SRAM_OBJ_CNT * ((x) + (y) * SRAM_USR_NUM) + 1))
#define SRAM_USR_VAL(x, y) \
	(SRAM_BASE + 4 * (2 + SRAM_OBJ_CNT * ((x) + (y) * SRAM_USR_NUM) + 2))
...
#define SRAM_PWR_SEC(x, y) \
	(SRAM_BASE + 4 * (79 + SRAM_OBJ_CNT * ((x) + (y) * SRAM_PWR_CNT) + 0))
#define SRAM_PWR_USEC(x, y) \
	(SRAM_BASE + 4 * (79 + SRAM_OBJ_CNT * ((x) + (y) * SRAM_PWR_CNT) + 1))
#define SRAM_PWR_VAL(x, y) \
	(SRAM_BASE + 4 * (79 + SRAM_OBJ_CNT * ((x) + (y) * SRAM_PWR_CNT) + 2))

/* set_parent : SRAM_OBJ_CNT * SRAM_PWR_CNT * SRAM_REC_CNT = 96 */
#define SRAM_REC_CNT_CLK(x)	(SRAM_BASE + 4 * (175 + (x)))
#define SRAM_CLK_SEC(x, y) \
	(SRAM_BASE + 4 * (179 + SRAM_OBJ_CNT * ((x) + (y) * SRAM_PWR_CNT) + 0))
#define SRAM_CLK_USEC(x, y) \
	(SRAM_BASE + 4 * (179 + SRAM_OBJ_CNT * ((x) + (y) * SRAM_PWR_CNT) + 1))
#define SRAM_CLK_VAL(x, y) \
	(SRAM_BASE + 4 * (179 + SRAM_OBJ_CNT * ((x) + (y) * SRAM_PWR_CNT) + 2))
...
#define SRAM_MUX_MIN(x)		(SRAM_BASE + 4 * (399 + (x)))
#define SRAM_PWR_GEAR(x)	(SRAM_BASE + 4 * (415 + (x)))
