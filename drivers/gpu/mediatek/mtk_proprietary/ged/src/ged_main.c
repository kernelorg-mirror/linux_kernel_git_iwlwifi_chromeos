...
#include "ged_type.h"
#include "ged_dcs.h"
//#include "mtk_drm_arr.h" // chromebook does not support it
...
bool g_ged_gpueb_support;
bool g_ged_fdvfs_support;
bool g_ged_gpu_freq_notify_support;
...
GED_ERROR check_eb_config(void)
{
	...
	if (!fdvfs_node) {
		...
		g_ged_gpu_freq_notify_support = false;
		g_ged_gpueb_support = false;
	} else {
		g_ged_fdvfs_support = of_property_read_bool(fdvfs_node, "fdvfs-policy-support");
		g_ged_gpu_freq_notify_support = of_property_read_bool(fdvfs_node,
				"gpu-freq-notify-support");
	}
...
static int ged_pdrv_probe(struct platform_device *pdev)
{
	...
	if (g_ged_gpueb_support) {
		...
		g_is_bringup = (!g_ged_gpu_freq_notify_support)? 1: 0;
