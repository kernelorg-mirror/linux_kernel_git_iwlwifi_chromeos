...
static bool vpu_dec_check_ap_inst(struct mtk_vcodec_dev *dec_dev, struct vdec_vpu_inst *vpu)
{
...
{
	struct mtk_vcodec_dev *dec_dev;
	const struct vdec_vpu_ipi_ack *msg = data;
	struct vdec_vpu_inst *vpu;

	dec_dev = (struct mtk_vcodec_dev *)priv;
	vpu = (struct vdec_vpu_inst *)(unsigned long)msg->ap_inst_addr;
	if (!priv || !vpu) {
		pr_err("ap_inst_addr is NULL, did the SCP hang or crash?");
		return;
	}

	if (!vpu_dec_check_ap_inst(dec_dev, vpu) || msg->msg_id < VPU_IPIMSG_DEC_INIT_ACK ||
	    msg->msg_id > VPU_IPIMSG_DEC_GET_PARAM_ACK) {
		mtk_vcodec_err(vpu, "vdec msg id not correctly => 0x%x", msg->msg_id);
		vpu->failure = -EINVAL;
		goto error;
	}
