...
#define GED_DVFS_FB_TIMER_TIMEOUT 16666666
...
void ged_eb_dvfs_trace_dump(void)
{
	...
	if (eb_policy_state != pre_eb_policy_state ||
		...
		trace_ged_tracing_mark_write(5566, "commit_type",
			eCommitType);

		trace_ged_tracing_mark_write(5566, "EB_commit_type",
			eb_policy_state);
		...
		trace_ged_tracing_mark_write(5566, "26m_replace",
			mtk_gpueb_sysram_read(SYSRAM_GPU_EB_26M_REPLACE));
	}

	if (eb_policy_state != POLICY_STATE_FB) {
		trace_ged_tracing_mark_write(5566, "t_gpu",
			mtk_gpueb_sysram_read(SYSRAM_GPU_EB_USE_T_GPU));
		trace_ged_tracing_mark_write(5566, "t_gpu_target",
	...
	if (eb_policy_state == GED_DVFS_LOADING_BASE_COMMIT) {
		...
		if (apply_lb_async) {
			trace_ged_tracing_mark_write(5566, "async_perf_high",
				mtk_gpueb_sysram_read(SYSRAM_GPU_EB_USE_PERF_IMPROVE));
		}
	}
	if (eb_policy_state != GED_DVFS_FRAME_BASE_COMMIT) {
		trace_ged_tracing_mark_write(5566, "async_opp_diff",
...
void ged_dvfs_gpu_clock_switch_notify(enum ged_gpu_power_state power_state)
{
	...
	trace_ged_tracing_mark_write(5566, "gpu_state", power_state);
	//MBrain
