 ...
 *==================================================
 */
static int lvts_raw_to_temp_v1(
	struct formula_coeff *co, unsigned int sensor_id, unsigned int msr_raw)
{
...
static unsigned int lvts_temp_to_raw_v1(
	struct formula_coeff *co, unsigned int sensor_id, int temp)
{
	unsigned int msr_raw = 0;

	msr_raw = div_s64(((long long)co->golden_temp * 500 - co->a[0] - temp) << 14,
			  -1 * co->a[0]);
...
static int lvts_read_tc_temperature(struct lvts_data *lvts_data, unsigned int tz_id, bool in_isr)
{
	...
	unsigned int i, j, msr_raw, s_index;
	int max_temp = THERMAL_TEMP_INVALID, current_temp;
	void __iomem *base;
	struct platform_ops *ops = &lvts_data->ops;

	/*
	 * If tz_id == 0: Iterate all sensors and return the maximum temperature.
	 * If tz_id > 0: Only read and return the temperature of the specified sensor (tz_id - 1).
	 */
	...
	return (tz_id == 0) ? max_temp : THERMAL_TEMP_INVALID;
}
...
static int soc_temp_lvts_read_temp(struct thermal_zone_device *tz, int *temperature)
{
	...
	if (lvts_tz->id < lvts_data->num_sensor + 1)
...
static void lvts_write_device(struct lvts_data *lvts_data, unsigned int data, int tc_id)
{
	...
	udelay(5);
	ret = readl_poll_timeout(LVTS_CONFIG_0 + base, data, !(data & DEVICE_ACCESS_STARTUS), 2,
				 200);
	if (ret)
...
static unsigned int lvts_read_device(struct lvts_data *lvts_data, unsigned int reg_idx, int tc_id)
{
	...
	ret = readl_poll_timeout(LVTS_CONFIG_0 + base, data, !(data & DEVICE_ACCESS_STARTUS), 2,
				 200);
	if (ret)
		dev_err(dev, "read device err: LVTS %d didn't ready, reg_idx 0x%x\n",
			tc_id, reg_idx);
...
static void enable_all_sensing_points(struct lvts_data *lvts_data)
{
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		...
		if (num > ALL_SENSING_POINTS) {
			dev_err(dev, "%s, LVTS%d, illegal number of sensors: %d\n",
...
static int lvts_thermal_check_all_sensing_point_idle(struct lvts_data *lvts_data)
{
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		...
		if ((temp & 0x481) != 0) {
			error_code = (i << 16) + ((temp & BIT(10)) >> 8) + ((temp & BIT(7)) >> 6) +
				     (temp & BIT(0));
...
static void lvts_wait_for_all_sensing_point_idle(struct lvts_data *lvts_data)
{
	 ...
	 * this will spend extra 160us of suspend flow.
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		...
		if ((temp & mask) != 0) {
			error_code = ((temp & BIT(10)) >> 8) + ((temp & BIT(7)) >> 6) +
				     (temp & BIT(0));

			dev_err(dev, "Error LVTS %d sensing points aren't idle, error_code %d\n",
				i, error_code);
		}
	}
}
...
{
	struct tc_settings *tc = lvts_data->tc;
	unsigned int lvts_mon_ctrl1, lvts_mon_ctrl2;
	void __iomem *base;

	base = GET_BASE_ADDR(tc_id);

	lvts_mon_ctrl1 = (tc[tc_id].tc_speed.group_interval_delay << 20 & GENMASK(29, 20)) |
			 (tc[tc_id].tc_speed.period_unit & GENMASK(9, 0));
	lvts_mon_ctrl2 = (tc[tc_id].tc_speed.filter_interval_delay << 16 & GENMASK(25, 16)) |
	 ...
	 */
	writel(lvts_mon_ctrl1, LVTSMONCTL1_0 + base);
	writel(lvts_mon_ctrl2, LVTSMONCTL2_0 + base);
}
...
static int get_dominator_index(struct lvts_data *lvts_data, int tc_id)
{
	...
	if (tc[tc_id].dominator_sensing_point == ALL_SENSING_POINTS) {
		...
		dev_err(dev,
			"LVTS%d, dominator_sensing_point= %d should smaller than num_sensor= %d\n",
...
static void update_all_tc_hw_reboot_point(struct lvts_data *lvts_data, int trip_point)
{
	struct tc_settings *tc = lvts_data->tc;
	int i;
...
static int soc_temp_lvts_set_trip_temp(struct thermal_zone_device *tz, int trip, int temp)
{
	struct soc_temp_tz *lvts_tz = (struct soc_temp_tz *)tz->devdata;
	struct lvts_data *lvts_data = lvts_tz->lvts_data;
...
static void check_cal_data_v1(struct lvts_data *lvts_data)
{
	...
	if (cal_data->golden_temp != 0) {
		...
		for (i = 0; i < lvts_data->num_sensor; i++) {
			if (cal_data->count_r[i] != 0 || cal_data->count_rc[i] != 0) {
				cal_data->use_fake_efuse = 0;
				break;
			}
		}
	...
	if (cal_data->use_fake_efuse) {
		...
		dev_err(dev, "[lvts_cal] This sample is not calibrated, fake !!\n");
...
static int lvts_init(struct lvts_data *lvts_data)
{
	...
	if (!lvts_data->clock_gate_no_need) {
		...
		if (ret) {
			dev_err(dev, "Error: Failed to enable lvts controller clock: %d\n", ret);
			return ret;
		}
	}
...
static int prepare_calibration_data(struct lvts_data *lvts_data)
{
	...
	cal_data->count_r = devm_kcalloc(dev, lvts_data->num_sensor, sizeof(*cal_data->count_r),
					 GFP_KERNEL);
	if (!cal_data->count_r)
		return -ENOMEM;

	cal_data->count_rc = devm_kcalloc(dev, lvts_data->num_sensor, sizeof(*cal_data->count_rc),
					  GFP_KERNEL);
...
static int get_calibration_data(struct lvts_data *lvts_data)
{
	...
	lvts_data->efuse = devm_kcalloc(dev, lvts_data->num_efuse_addr, sizeof(*lvts_data->efuse),
					GFP_KERNEL);
	if (!lvts_data->efuse)
		return -ENOMEM;
...
static int of_update_lvts_data(
	struct lvts_data *lvts_data, struct platform_device *pdev)
{
	struct device *dev = lvts_data->dev;
	...
	lvts_data->sen_data = devm_kcalloc(dev, lvts_data->num_sensor, sizeof(*lvts_data->sen_data),
					   GFP_KERNEL);
...
static void tc_irq_handler(struct lvts_data *lvts_data, int tc_id)
{
	...
	unsigned int ret = 0;
	void __iomem *base;
	int temp = lvts_read_tc_temperature(lvts_data, 0, true);
	...
	dev_dbg(dev, "[Thermal IRQ] LVTS thermal controller %d, LVTSMONINTSTS=0x%08x, T=%d\n",
		tc_id, ret, temp);

	if (ret & THERMAL_HIGH_OFFSET_INTERRUPT_0)
		dev_dbg(dev, "[Thermal IRQ]: Thermal high offset0 interrupt, sw reset\n");
	if (ret & THERMAL_HIGH_OFFSET_INTERRUPT_1)
		dev_dbg(dev, "[Thermal IRQ]: Thermal high offset1 interrupt, sw reset\n");
	if (ret & THERMAL_HIGH_OFFSET_INTERRUPT_2)
		dev_dbg(dev, "[Thermal IRQ]: Thermal high offset2 interrupt, sw reset\n");
	if (ret & THERMAL_HIGH_OFFSET_INTERRUPT_3)
		dev_dbg(dev, "[Thermal IRQ]: Thermal high offset3 interrupt, sw reset\n");

	if (ret & THERMAL_PROTECTION_STAGE_3)
		dev_dbg(dev,
			"[Thermal IRQ]: Thermal protection stage 3 interrupt, HW reboot\n");
}
...
static int lvts_register_irq_handler(struct lvts_data *lvts_data)
{
	...
	irq_bitmap = devm_kcalloc(dev, lvts_data->num_domain, sizeof(*irq_bitmap), GFP_KERNEL);
	if (!irq_bitmap)
		return -ENOMEM;
...
static int lvts_register_thermal_zone(
	int id, struct lvts_data *lvts_data, struct thermal_zone_device **tzdev)
{
	struct device *dev = lvts_data->dev;
	...
	*tzdev = devm_thermal_of_zone_register(dev, lvts_tz->id, lvts_tz, &soc_temp_lvts_ops);
...
static int lvts_probe(struct platform_device *pdev)
{
	...
	if (lvts_data->init_in_lk)
		dev_dbg(dev, "[Thermal/LVTS] init done in lk2\n");
	else {
...
static int lvts_suspend_noirq(struct device *dev)
{
	...
	dev_dbg(dev, "[Thermal/LVTS]%s, suspend_support = %d\n", __func__,
...
static int device_read_count_rc_n_v4(struct lvts_data *lvts_data)
{
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		...
		for (j = 0; j < tc[i].num_sensor; j++) {
			...
			if (ret)
				dev_err(dev, "Error: LVTS %d DEVICE_SENSING_STATUS didn't ready\n",
					i);
