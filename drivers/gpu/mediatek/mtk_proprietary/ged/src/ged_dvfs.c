...
static unsigned int gpu_util_history_query_loading(unsigned int window_size_us ,int high_step,int low_step)
{
	...
	if (loading_mode == LOADING_ACTIVE && window_size_us == g_loading_slide_window_size * 1000
		...
		trace_ged_tracing_mark_write(5566, "26m_replace",1);
		return window_avg_loading_comp;
	}

	trace_ged_tracing_mark_write(5566, "26m_replace",0);
...
unsigned int ged_dvfs_write_sysram_protm_enter(void)
{
	...
	protm_enable = 1;
	trace_ged_tracing_mark_write(5566, "p-mode",protm_enable);
	...
	unsigned long ui32NewFreq, GED_DVFS_COMMIT_TYPE eCommitType)
{
	...
	if (ged_dvfs_gpu_freq_commit_fp != NULL) {
		...
		trace_ged_tracing_mark_write(5566, "gpu_freq",
		...
		trace_ged_tracing_mark_write(5566, "gpu_freq_ceil",
			(long long) div_u64(ged_get_freq_by_idx(ui32CeilingID), 1000));
		trace_ged_tracing_mark_write(5566, "gpu_freq_floor",
			(long long) div_u64(ged_get_freq_by_idx(ui32FloorID), 1000));
		trace_ged_tracing_mark_write(5566, "limitter_ceil",
			ged_get_cur_limiter_ceil());
		trace_ged_tracing_mark_write(5566, "limitter_floor",
			ged_get_cur_limiter_floor());
		if (ged_get_cur_limiter_ceil() == LIMIT_POWERHAL) {
			trace_ged_tracing_mark_write(5566, "limitter_ceil_pid",
				g_cust_upbound_freq_id_info.pid);
			trace_ged_tracing_mark_write(5566, "limitter_ceil_id",
				g_cust_upbound_freq_id_info.user_id);
			trace_ged_tracing_mark_write(5566, "limitter_ceil_cus_val",
				g_cust_upbound_freq_id_info.value);
		}

		if (ged_get_cur_limiter_floor() == LIMIT_POWERHAL) {
			trace_ged_tracing_mark_write(5566, "limitter_floor_pid",
				g_cust_boost_freq_id_info.pid);
			trace_ged_tracing_mark_write(5566, "limitter_floor_id",
				g_cust_boost_freq_id_info.user_id);
			trace_ged_tracing_mark_write(5566, "limitter_floor_cus_val",
				g_cust_boost_freq_id_info.value);
		}
		trace_ged_tracing_mark_write(5566, "commit_type", eCommitType);
		if (dcs_get_adjust_support() % 2 != 0)
			trace_ged_tracing_mark_write(5566, "preserve", g_force_disable_dcs);

		trace_ged_tracing_mark_write(5566, "fix", is_fix_dvfs);
	...
	unsigned long topNewFreqID)
{
	...
	if (eCommitType != GED_DVFS_EB_DESIRE_COMMIT)
		...
		trace_ged_tracing_mark_write(5566, "eb_update_vir", stackNewFreqID);
	if (dcs_get_adjust_support() % 2 != 0)
		trace_ged_tracing_mark_write(5566, "preserve", g_force_disable_dcs);
...
increaseEnd:
	...
	trace_ged_tracing_mark_write(5566, "async_perf_high", perf_improve);
	...
	int target_fps_margin, unsigned int force_fallback)
{
	...
	trace_GPU_DVFS__Policy__Frame_based__GPU_Time((t_gpu * 100),
								  ...
								  (t_gpu_real * 100),
								  (t_gpu_pipe * 100));
	trace_ged_tracing_mark_write(5566, "t_gpu", (long long) t_gpu * 100);
	trace_ged_tracing_mark_write(5566, "t_gpu_target", (long long) t_gpu_target * 100);
	...
	if (g_gpu_timer_based_emu) {
		...
		if (ret_risky_bq == GED_OK) {
			...
			if (g_max_core_num == SHADER_CORE &&
				gx_dvfs_loading_mode == LOADING_MAX_ITERMCU &&
				early_force_fallback_enable)
				trace_ged_tracing_mark_write(5566, "early_force_fallback"
		...
		trace_ged_tracing_mark_write(5566, "t_gpu", t_gpu);
		trace_ged_tracing_mark_write(5566, "t_gpu_target", t_gpu_target);
		...
		trace_ged_tracing_mark_write(5566, "async_opp_diff", g_async_opp_diff);
...
GED_ERROR ged_dvfs_system_init(void)
{
	...
	dvfs_loading_mode_node = of_find_compatible_node(NULL, NULL, "mediatek,gpu-loading-mode");
