...
static void ged_eb_work_cb(struct work_struct *psWork)
{
	...
	case GPUFDVFS_IPI_EVENT_CLK_CHANGE:
		...
		if (psEBEvent->freq_new < ged_get_top_freq_by_virt_opp(ged_get_min_oppidx_real()) - (EB_CLOCK*1000*2)){
			trace_ged_tracing_mark_write(5566, "unreasonable_top_freq",psEBEvent->freq_new);
		} else {
		...
		GED_DVFS_COMMIT_TYPE eCommitType, int *pbCommited)
{
	...
	if (ulNewFreqID != ulPreFreqID) {
		...
		/* mtk_gpueb_sysram_write(SYSRAM_GPU_COMMIT_PLATFORM_FREQ_IDX, ulNewFreqID);
		 * mtk_gpueb_sysram_write(SYSRAM_GPU_COMMIT_TYPE, eCommitType);
		 * mtk_gpueb_sysram_write(SYSRAM_GPU_COMMIT_VIRTUAL_FREQ, 0);
		 */
#else
...
#ifdef FDVFS_REDUCE_IPI
		/* mtk_gpueb_sysram_write(SYSRAM_GPU_COMMIT_PLATFORM_FREQ_IDX, platform_freq_idx);
		 * mtk_gpueb_sysram_write(SYSRAM_GPU_COMMIT_TYPE, eCommitType);
		 * mtk_gpueb_sysram_write(SYSRAM_GPU_COMMIT_VIRTUAL_FREQ, virtual_freq_in_MHz);
		 */
#else
		struct fdvfs_ipi_data ipi_data;
...
#ifdef FDVFS_REDUCE_IPI
		/* mtk_gpueb_sysram_write(SYSRAM_GPU_SET_TARGET_FRAME_TIME, target_frame_time);
		 * mtk_gpueb_sysram_write(SYSRAM_GPU_SET_TARGET_MARGIN, target_margin);
		 */
#else
		struct fdvfs_ipi_data ipi_data;
