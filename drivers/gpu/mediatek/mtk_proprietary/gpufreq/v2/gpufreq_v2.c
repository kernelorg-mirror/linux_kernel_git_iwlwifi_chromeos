...
static const struct of_device_id g_gpufreq_wrapper_of_match[] = {
	{ .compatible = "mediatek,gpufreq-wrapper" },
	{ /* sentinel */ }
};
...
static bool g_gpueb_support;
static bool g_gpufreq_bringup;
...
static int gpufreq_shared_memory_init(void)
{
	...
	struct device_node *of_gpueb_rmem_node = NULL;
	...
	if (unlikely(!gpueb_mem_pa) || unlikely(!gpueb_mem_size)) {
		/* use dedicated memory */
		of_gpueb_rmem_node = of_find_compatible_node(NULL, NULL, "mediatek,gpueb-resv-mem");
		if (!of_gpueb_rmem_node) {
			GPUFREQ_LOGE("no node for gpueb-resv-mem. (%d)", __LINE__);
			return -EINVAL;
		}

		rmem = of_reserved_mem_lookup(of_gpueb_rmem_node);
		of_node_put(of_gpueb_rmem_node);

		if (!rmem) {
			GPUFREQ_LOGE("cannot lookup gpueb-resv-mem. (%d)", __LINE__);
			return -EINVAL;
		}
		gpueb_mem_pa   = rmem->base;
		gpueb_mem_size = rmem->size;
	}

	if (unlikely(!gpueb_mem_pa) || unlikely(!gpueb_mem_size)) {
		GPUFREQ_LOGE("fail to get gpueb memory info: gpueb_mem_pa (0x%llx), gpueb_mem_size (0x%llx)",
			     gpueb_mem_pa, gpueb_mem_size);
		ret = GPUFREQ_ENOENT;
		goto done;
	}
...
static int gpufreq_wrapper_pdrv_probe(struct platform_device *pdev)
{
	...
	g_gpueb_support = of_property_read_bool(of_wrapper, "gpueb-support");
	g_gpufreq_bringup = of_property_read_bool(of_wrapper, "gpufreq-bringup");
