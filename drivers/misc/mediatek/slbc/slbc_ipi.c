...
int _slbc_ach_scmi(unsigned int cmd, enum slc_ach_uid uid, int gid, struct slbc_gid_data *data)
{
	...
	if (cmd == IPI_SLBC_GID_REQUEST_FROM_AP) {
		...
		slbc_ipi_d.arg3		= data->bw;	/* reuse bw as enable argument */
	}
...
static void slbc_scmi_handler(u32 r_feature_id, struct scmi_tinysys_report_st *report)
{
	struct slbc_data d;
	unsigned int cmd;
	unsigned int arg;
	...
	case IPI_SLBC_ACP_REQUEST_TO_AP:
		...
		if (d.type == TP_ACP) {
			...
			pr_err("#@# %s(%d) wrong cmd(%s) and type(0x%x)\n",
					__func__, __LINE__,
	...
	case IPI_SLBC_DCC_CTRL_TO_AP:
		...
		break;
	default:
		pr_err("wrong slbc IPI command: %d\n",
				cmd);
	}
}
...
int slbc_scmi_ctrl(void *buffer, void *ptr)
{
	...
	if (slbc_sspm_ready != 1) {
		ret = -ENOT_AVAILABLE;
		pr_err("slbc scmi not ready, skip cmd=%d\n", slbc_ipi_d->cmd);
		goto error;
	}
	...
	if (ret) {
		pr_err("slbc scmi cmd %d send fail, ret = %d\n",
				slbc_ipi_d->cmd, ret);
...
int slbc_scmi_init(void)
{
	...
	if (!(_tinfo && _tinfo->sdev)) {
		pr_err("slbc call get_scmi_tinysys_info() fail\n");
		return -EPROBE_DEFER;
	}
	...
	ret = of_property_read_u32(_tinfo->sdev->dev.of_node, "scmi-slbc",
			&scmi_slbc_id);
	if (ret) {
		pr_err("get slbc scmi_slbc fail, ret %d\n", ret);
		slbc_sspm_ready = -2;
		return -EINVAL;
	}
